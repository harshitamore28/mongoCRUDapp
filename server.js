...
const connectionString = 'mongodb+srv://harshita:indegene@cluster0.xsiol.mongodb.net/myFirstDatabase?retryWrites=true&w=majority';
...
MongoClient.connect(connectionString, (err, client) => {
    ...
    serverApp.post('/createEmployee', (req, res) => {
        ...
        insertOne(req.body)
            .then(result => {
                res.redirect('/')
                res.send('Info created') //put in then
            })
            //.then(result => {
    ...
    serverApp.put('/updateEmployee', (req, res) => {
        employeeCollection.findOneAndUpdate({ //parametes 1what to find, 2what to send, 3update
                name: req.body.name,
            }, {
                $set: {
                    name: 'Revanth',
